...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        m.parse_file(input);
        NFA automata = m.combine_NFA();
        DFA temp = m.convert_to_DFA(automata);
        temp = m.minimize_DFA(temp);
        File test = new File("test.txt");
        try {
            m.scan_input(test,temp);
        }catch (Exception e ){
            System.out.println("problem with scan input");
    ...
    public void scan_input(File file , DFA dfa) throws Exception {
        ...
        String str ,lastAcceptState="";

        DFA_State currentState = dfa.getStart_state();
        ...
        while ((str = br.readLine()) != null){
            while (indx < str.length() || !queue.isEmpty()){
                ...
                if(currentState != null && dfa.checkAccpetState(currentState)){
                    lastAcceptState = dfa.getToken(currentState);
                    queue.clear();
                 ...
                 */
                    ...
                    currentState = dfa.getStart_state();
                 ...
                 */
                else if(currentState == null || (!dfa.checkAccpetState(currentState) && indx >= str.length() ) ){
                    if(currentChar != ' ')
                    {
                        bw.write("Error");
                        bw.newLine();
